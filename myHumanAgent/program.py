...
class Agent:
    ...
    def action(self, **referee: dict) -> Action:
        ...
        """
        game_action = None
        while(game_action == None):
            sleep(0.016)

            # keep reading game info, unti we get input from user
            try:
                with open("gameInfo.json","r") as fp:
                        data = json.load(fp)
                        if(data is None):
                            continue
                        coord = Coord(data["row"],data["col"])        
                        if(data["action"] == "place"):
                            game_action=PlaceAction(coord)
                        elif(data["action"] == "move"):
                            game_action=MoveAction(coord,Direction[data["direction"]])
                        elif(data["action"] == "eat"):
                            game_action=EatAction(coord,Direction[data["direction"]])
                        else:
                            game_action=CascadeAction(coord,Direction[data["direction"]])
            except:
                continue
